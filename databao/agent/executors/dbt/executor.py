...
from databao.agent.databases.databases import db_type, register_db_in_duckdb
from databao.agent.executors.base import GraphExecutor
from databao.agent.executors.dbt.config import DbtConfig
from databao.agent.executors.dbt.dbt_runner import (
    PostDbtRunHook,
    assemble_dbt_project_summary,
    duckdb_post_run_hook,
)
from databao.agent.executors.dbt.errors import DbtMissingWarehouseError
from databao.agent.executors.dbt.graph import DbtProjectGraph
from databao.agent.executors.dbt.query_runner import DuckDbQueryRunner
...
class DbtProjectExecutor(GraphExecutor):
    ...
    def __init__(
        ...
        *,
        ...
        self._prompt_template = load_prompt_template("databao.agent.executors.dbt", "system_prompt.jinja")
        self._task_instruction = load_prompt_template("databao.agent.executors.dbt", "task_instruction.jinja").render()
